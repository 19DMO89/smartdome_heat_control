...
async def async_setup_entry(
    ...
    for room_id, room in rooms.items():
        ...
        entities.append(SmartdomeRoomAwaySwitch(hass, entry, room_id, label))
...
class SmartdomeRoomBaseSwitch(SmartHeatingBaseSwitch):
    ...
    def _apply_room_config_update(self, room_key: str, value: bool) -> None:
        ...
        apply_room_config_update(self.hass, self._entry, self._room_id, {room_key: value})
        self.async_write_ha_state()
...
class SmartdomeRoomAwaySwitch(SmartdomeRoomBaseSwitch):
    """Switch: Away-Modus für diesen Raum (unabhängig vom globalen Away-Modus)."""
    ...
    def __init__(
        ...
        super().__init__(hass, entry, room_id, room_label)
        self._attr_unique_id = f"{entry.entry_id}_room_{room_id}_away"
        self._attr_name = f"{room_label} – Away Modus"
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        self._apply_room_config_update(CONF_ROOM_AWAY_ENABLED, True)

    async def async_turn_off(self, **kwargs: Any) -> None:
        self._apply_room_config_update(CONF_ROOM_AWAY_ENABLED, False)
