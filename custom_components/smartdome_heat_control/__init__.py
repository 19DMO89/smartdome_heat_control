...
def _normalize_rooms(rooms: Any) -> dict[str, dict[str, Any]]:
    ...
    for room_id, room in rooms.items():
        ...
        normalized[room_id] = {
            ...
            CONF_ROOM_CLIMATE_ENTITY: str(room.get(CONF_ROOM_CLIMATE_ENTITY, "")),
            CONF_ROOM_USE_CLIMATE: bool(room.get(CONF_ROOM_USE_CLIMATE, False)),
            CONF_ROOM_COOL_TARGET_DAY: float(room.get(CONF_ROOM_COOL_TARGET_DAY, DEFAULT_ROOM_COOL_TARGET_DAY)),
            CONF_ROOM_COOL_TARGET_NIGHT: float(room.get(CONF_ROOM_COOL_TARGET_NIGHT, DEFAULT_ROOM_COOL_TARGET_NIGHT)),
            CONF_ROOM_CLIMATE_FAN_MODE: str(room.get(CONF_ROOM_CLIMATE_FAN_MODE, DEFAULT_ROOM_CLIMATE_FAN_MODE)),
            CONF_ROOM_CLIMATE_PRESET: str(room.get(CONF_ROOM_CLIMATE_PRESET, DEFAULT_ROOM_CLIMATE_PRESET)),
            CONF_ROOM_CLIMATE_HVAC_MODE: str(room.get(CONF_ROOM_CLIMATE_HVAC_MODE, DEFAULT_ROOM_CLIMATE_HVAC_MODE)),
        }
...
def _normalize_config(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized.setdefault(CONF_COOLING_ENABLED, DEFAULT_COOLING_ENABLED)
    normalized.setdefault(CONF_COOLING_AUTO, DEFAULT_COOLING_AUTO)
    normalized.setdefault(CONF_COOLING_THRESHOLD, DEFAULT_COOLING_THRESHOLD)
    return normalized
...
def _async_register_services(hass: HomeAssistant) -> None:
    ...
    async def handle_update_config(call: ServiceCall) -> None:
        ...
        # Rooms and circuits must be replaced entirely, not merged, so that
        # deleted entries are actually removed and don't reappear after reload.
        if CONF_ROOMS in patch:
            current_cfg[CONF_ROOMS] = patch.pop(CONF_ROOMS)
        if CONF_CIRCUITS in patch:
            current_cfg[CONF_CIRCUITS] = patch.pop(CONF_CIRCUITS)
